...
mkdir -p $WALTOLAND_WALLPAPER_DIR
...
purgeWallpaperDir() {
  find "${WALTOLAND_WALLPAPER_DIR}" -type f -mtime +14 -exec rm -f {} \;
  echo "Deleted all files older than two weeks in ${WALTOLAND_WALLPAPER_DIR}"
}
...
downloadBingImage() {
  ...
  local filePathBase="${WALTOLAND_WALLPAPER_DIR}/${fileNameBase}"
...
if $START ; then
  systemctl --user start waltoland-wallpaper-service.service
else
  while true; do
    NEW_IMAGE_FILE=""
    SLEEP_TIMEOUT=1
    ...
    if ! swww query &>/dev/null; then
      ...
      if [ -n "${NEW_IMAGE_FILE}" ] ; then 
        ...
        NEW_IMAGE_FILE=$(find "$WALTOLAND_WALLPAPER_DIR" -type f -name "*.jpg" | shuf -n 1)
